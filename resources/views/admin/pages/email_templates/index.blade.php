    ...
    <div class="content">
        <div class="card">
            <div class="card-header header-elements-inline">
                <h5 class="card-title">Search email template</h5>
                    ...
                    <div class="form-row">
                        ...
                        <div class="form-group col-sm-3">
                            ...
                            <input type="text" class="form-control" name="slug" id="slug"
                                   placeholder="{{ __('admin.slug') }}" value="{{request('slug')}}">
                        </div>
                        <div class="form-group col-sm-3">
    ...
    <!-- Basic table -->
        <div class="card" style="zoom: 1;">
            <div class="card-header row">
                <div class="col-md-4 mr-1 mb-2 border p-1 rounded">
                    ...
                    <button class="btn btn-success status_all col-md-5" data-type="1"
                            data-url="{{ url('statusMultipleEmailTemplate') }}">
                        Enable Selected
                    </button>
            ...
            <div class="table-responsive" style="">
                <table class="table table-striped table-bordered">
                    ...
                    @foreach($emailTemplates as $emailTemplate)
                        <tr id="tr_{{$emailTemplate->id}}">
                            <td><input type="checkbox" class="sub_chk" data-id="{{$emailTemplate->id}}"></td>
                            <td>{{ $emailTemplate->id }}</td>
                            <td>{{ $emailTemplate->title }}</td>
                            <td>{{ $emailTemplate->subject_en }}</td>
                            <td>{{ $emailTemplate->slug }}</td>
                            <td>
                                <select name="status" id="status" data-email-template-id="{{ $emailTemplate->id }}">
                                    @foreach($status as $statusKey => $statusVal)
                                        <option
                                            {{ $statusKey == $emailTemplate->status ? 'selected': '' }} value="{{ $statusKey }}">{{ $statusVal }}</option>
                                    @endforeach
                                </select>
                            </td>
            ...
            <div class="card-footer bg-white d-flex justify-content-between align-items-center">
                ...
                <span class="text-muted">{{ ($emailTemplates->currentPage() * config('global.pagination_count') >= $emailTemplates->total()) ? ($emailTemplates->currentPage() * config('global.pagination_count') - config('global.pagination_count')) ." - ". $emailTemplates->total() : ($emailTemplates->currentPage() * config('global.pagination_count') - config('global.pagination_count')) ." - ". $emailTemplates->currentPage() * config('global.pagination_count') }}
                        /
...
@section('script')
    <script>
        $('select#status').focus(function () {
            ...
            let $this = $(this);
            let status = $this.val();
            let email_template_id = $this.data('email-template-id');

            if (status > 0 && email_template_id > 0) {
                swal.fire({
                    title: 'Alert',
                    text: "Do you want change status this template!",
                    type: 'warning',
                        ...
                        $.ajax({
                            ...
                            success: function (data) {
                                if (data.response.status == 'OK') {
                                    swal.fire({
                                        title: 'Success',
                                        text: "This template's status was changed successfully!",
                                        type: 'success',
