    ...
    <div class="content">
        <div class="card">
            <div class="card-body">
                ...
                <form action="{{route($module_name.'.cms_users.store')}}" method="POST" enctype="multipart/form-data">
                    @csrf
                    <div class="form-row">
                        ...
                        <div class="form-group col-sm-6">
                            ...
                            <div class="form-group-feedback form-group-feedback-right">
                                <input type="text" name="lastname" id="lastname" class="form-control @error('lastname') border-danger @enderror" placeholder="{{__('admin.lastname')}}" value="{{ old('lastname') }}">
                            </div>
                            @error('lastname')
                            <span class="form-text font-weight-semibold text-danger">{{ $message }}</span>
                            @enderror
                        ...
                        <div class="form-group col-sm-6">
                            ...
                            <div class="form-group-feedback form-group-feedback-right">
                                <input type="text" name="email" id="email" class="form-control @error('email') border-danger @enderror" placeholder="{{__('admin.email')}}" value="{{ old('email') }}">
                            </div>
                            @error('email')
                            <span class="form-text font-weight-semibold text-danger">{{ $message }}</span>
                            @enderror
                            ...
                            <div class="form-group-feedback form-group-feedback-right">
                                <select name="store_ids[]" class="select-search" id="store_ids" multiple>
                                    ...
                                    @foreach($stores as $store)
                                        <option {{is_array(old('store_ids')) && in_array($store->id,old('store_ids')) ? 'selected': ''}} value="{{$store->id}}">{{$store->name}}</option>
                                    @endforeach
                                </select>
                            </div>
                            @error('store_ids')
                        ...
                        <div class="form-group col-sm-6">
                            ...
                            <div class="form-group-feedback form-group-feedback-right">
                                <input name="never_expire" type="checkbox" class="@error('never_expire') border-danger @enderror" id="never_expire" value="1" {{ old('never_expire') ? 'checked' : '' }}>
                            </div>
                        </div>
                        <div class="form-group col-sm-6 expire_date" style="display: none;">
                            <label class="font-weight-semibold @error('expire_date') text-danger @enderror" for="expire_date">{{__('admin.expire_date')}}</label>
                            <div class="form-group-feedback form-group-feedback-right">
                                @php
                                    if(old('expire_date'))
                                    {
                                        echo '<input type="text" name="expire_date" id="expire_date" class="form-control daterange-single"
