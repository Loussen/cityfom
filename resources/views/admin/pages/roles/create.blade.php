    ...
    <div class="content">
        <div class="card">
            <div class="card-body">
                ...
                <form action="{{route($module_name.'.roles.store')}}" method="POST" enctype="multipart/form-data">
                    @csrf
                    <div class="form-row">
                        ...
                        <div class="form-group col-sm-6">
                            ...
                            @foreach($permissions as $permission)
                                <br>
                                <div class="custom-control custom-checkbox custom-control-inline">
                                    <input name="permission[]" {{ ( is_array(old('permission')) && in_array($permission->id, old('permission')) ) ? 'checked ' : '' }} type="checkbox" class="custom-control-input sub_chk" id="custom_checkbox_inline_unchecked_{{$permission->id}}"  value="{{ $permission->id }}">
                        ...
                        <div class="col-sm-12">
                            <x-save />
                            <x-back route="{{$module_name}}.roles.index"></x-back>
                        </div>
                    </div>
                </form>
            </div>
