...
class CouponsController extends ApiController
{
    ...
    public function all(Request $request)
    {
        ...
        if($request->has('latitude') && $request->has('longitude'))
        {
            ...
            $getCouponPromote = DB::table('stores AS s')
                ->selectRaw('*, c.id as c_id, c.radius AS coupon_radius, c.title as c_title, c.image as c_image, ( 6371 * acos( cos( radians('.$latitude.') ) * cos( radians( latitude ) ) * cos( radians( longitude ) - radians('.$longitude.') ) + sin( radians('.$latitude.') ) * sin( radians( latitude ) ) ) ) AS distance, s.name AS store_name')
                ->join('coupons AS c','c.store_id','=','s.id')
                ->whereRaw("s.verified = ".config("global.enable")." AND s.status = ".config("global.enable")." AND c.status = ".config("global.enable")." AND valid_from <= NOW() AND valid_to >= NOW() AND UNIX_TIMESTAMP(c.start_date_promote) <= ".strtotime(date('Y-m-d H:i'))." AND UNIX_TIMESTAMP(c.end_date_promote) >= ".strtotime(date('Y-m-d H:i')))
                ->havingRaw('distance <= coupon_radius / 1000')
        ...
        if ($getCoupon->count() > 0 || $getCouponPromote->count() > 0) {
    ...
    public function detail(Request $request)
    {
        ...
        $getCoupon = DB::table('coupons AS c')
                    ...
                    ->where('s.status',config("global.enable"))
                    ->where('s.verified',config("global.enable"))
                    ->where('c.id','=',$validatedData['coupon_id'])
                    ->first();
